...
function visualize_traj_est_v1(IDX_data_packet, ...
                               stt_var_runtime, ...
...
% traj_EST: (nx3) 
% traj_GT:  (nx4), 1:time, 2,3,4: x,y,z
...
format_fname_img_res = './res_img/img_res_%d.pdf';
...
%%%% draw traj(GT)
if isempty(traj_GT) == 0
    x_traj = traj_GT(:, 2);
    y_traj = traj_GT(:, 3);
    z_traj = traj_GT(:, 4);

    plot3(x_traj, y_traj, z_traj, '-', 'LineWidth', 1.0, 'Color', [0.0, 0.8, 0.0]);
    %plot3(x_traj, y_traj, z_traj, 'o', 'MarkerSize', 1, 'Color', [0.0, 0.8, 0.0], 'MarkerFaceColor', [0.0, 0.8, 0.0]);
end



%%%% draw traj(EST)
if isempty(traj_EST) == 0
    x_traj = traj_EST(:, 1);
    y_traj = traj_EST(:, 2);
    z_traj = traj_EST(:, 3);

    plot3(x_traj, y_traj, z_traj, '-', 'LineWidth', 1.0, 'Color', [0.0, 0.0, 1.0]);
    %plot3(x_traj, y_traj, z_traj, 'o', 'MarkerSize', 1, 'Color', [0.0, 0.0, 1.0], 'MarkerFaceColor', [0.0, 0.0, 1.0]);
