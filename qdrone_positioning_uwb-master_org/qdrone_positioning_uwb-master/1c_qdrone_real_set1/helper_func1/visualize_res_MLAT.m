...
function visualize_res_MLAT(IDX_data_packet, Time_now, ...
                            Traj_origin_GT, Mat_body_rot_GT, Mat_body_trans_GT, ...
                            Traj_origin_EST1, Traj_origin_EST2, ...
...
%
%   Traj_origin_EST1          : (B x 4), 4: time,x,y,z  -> estimated by MLAT1
%   Traj_origin_EST2          : (B x 4), 4: time,x,y,z  -> estimated by MLAT2
%
...
format_fname_img_res = './screenshot/img_res_MLAT_%d.png';
...
if size(Traj_origin_GT, 1) >= 1,
    ...
    plot3(x_traj, y_traj, z_traj, 'o', 'MarkerSize', 2, 'Color', [0.0, 0.8, 0.0], 'MarkerFaceColor', 'r');   
    %plot3(x_traj, y_traj, z_traj, '.', 'Color', [1.0, 0.0, 0.0], 'LineWidth', 1.5);
    %plot3(x_traj, y_traj, z_traj, 'g-', 'LineWidth', 1.5);
end

% %%%% draw traj(EST1)
% if size(Traj_origin_EST1, 1) >= 1,
%     x_traj = Traj_origin_EST1(:, 2);
%     y_traj = Traj_origin_EST1(:, 3);
%     z_traj = Traj_origin_EST1(:, 4);
%     plot3(x_traj, y_traj, z_traj, 'o', 'MarkerSize', 2, 'Color', 'r', 'MarkerFaceColor', 'b');
%     %plot3(x_traj, y_traj, z_traj, 'b-', 'LineWidth', 1.5);
% end

%%%% draw traj(EST2)
if size(Traj_origin_EST2, 1) >= 1,
    x_traj = Traj_origin_EST2(:, 2);
    y_traj = Traj_origin_EST2(:, 3);
    z_traj = Traj_origin_EST2(:, 4);
